	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="服务进度" prop="fuwujindu">
					<el-select :disabled="ro.fuwujindu" v-model="ruleForm.fuwujindu" placeholder="请选择服务进度" >
						<el-option
							v-for="(item,index) in fuwujinduOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="服务进度" prop="fuwujindu">
					<el-input v-model="ruleForm.fuwujindu"
						placeholder="服务进度" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="健康状况" prop="jiankangzhuangkuang">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="健康状况" prop="jiankangzhuangkuang">
					<el-input v-model="ruleForm.jiankangzhuangkuang"
						placeholder="健康状况" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="时间" prop="shijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.shijian" label="时间" prop="shijian">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="治疗方案" prop="zhiliaofangan">
					<editor 
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.zhiliaofangan" label="治疗方案" prop="zhiliaofangan">
                    <span :style='{"fontSize":"14px","lineHeight":"1.5","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.zhiliaofangan"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fuwujindu'){
							this.ruleForm.fuwujindu = obj[o];
							this.ro.fuwujindu = true;
							continue;
						}
            ...
            this.jiankangzhuangkuangOptions = "健康,良好,一般,差".split(',')
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.zhiliaofangan = this.ruleForm.zhiliaofangan.replace(reg,'../../../springbootkpvj4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fuwujindu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fuwujinduCrossAddOrUpdateFlag = false;
						 this.parent.search();
