			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.chongwuzhonglei" type="text" placeholder="宠物种类" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"-8rpx 0px 0px rgba(188,188,226,.9)","padding":"16rpx","margin":"0 0 20rpx","backgroundColor":"#ecebf6","borderRadius":"12rpx","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"8rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('chongwuzhonglei','修改')) || (!userid && isAuthFront('chongwuzhonglei','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('chongwuzhonglei','删除')) || (!userid && isAuthFront('chongwuzhonglei','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.chongwuzhonglei="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.chongwuzhonglei){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`chongwuzhonglei`, params);
                } else {
                    res = await this.$api.list(`chongwuzhonglei`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chongwuzhonglei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.chongwuzhonglei){
					searchForm['chongwuzhonglei'] = '%' + this.searchForm.chongwuzhonglei + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`chongwuzhonglei`, searchForm);
                } else {
                    res = await this.$api.list(`chongwuzhonglei`, searchForm);
                }
				// 如果是第一页数据置空
