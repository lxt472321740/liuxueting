		...
		<view :style='{"width":"100%","padding":"0 0 64rpx","position":"relative","background":"url(http://codegen.caihongy.cn/20221108/581bb1956ffe4c8182127ca1e196e95e.png) fixed","height":"auto"}'>
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.binglimingcheng" type="text" placeholder="病例名称" ></input>
					</view>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"-8rpx 0px 0px rgba(188,188,226,.9)","padding":"16rpx","margin":"0 0 20rpx","backgroundColor":"#ecebf6","borderRadius":"12rpx","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#333","borderRadius":"8rpx 8rpx 0 0","background":"#fff","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>{{product.binglimingcheng}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"8rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('binglixinxi','修改')) || (!userid && isAuthFront('binglixinxi','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('binglixinxi','删除')) || (!userid && isAuthFront('binglixinxi','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.binglimingcheng="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.binglimingcheng){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`binglixinxi`, params);
                } else {
                    res = await this.$api.list(`binglixinxi`, params);
                }
				// 如果是第一页数据置空
			...
			async search(){
				...
				if(this.searchForm.binglimingcheng){
					searchForm['binglimingcheng'] = '%' + this.searchForm.binglimingcheng + '%'
				}
				if(this.searchForm.yonghuxingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`binglixinxi`, searchForm);
                } else {
                    res = await this.$api.list(`binglixinxi`, searchForm);
                }
				// 如果是第一页数据置空
