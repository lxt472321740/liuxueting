			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"-8rpx 0px 0px rgba(188,188,226,.9)","padding":"16rpx","margin":"0 0 20rpx","backgroundColor":"#ecebf6","borderRadius":"12rpx","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"8rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwuxuanze','修改')) || (!userid && isAuthFront('fuwuxuanze','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('fuwuxuanze','删除')) || (!userid && isAuthFront('fuwuxuanze','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`fuwuxuanze`, params);
                } else {
                    res = await this.$api.list(`fuwuxuanze`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('fuwuxuanze', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`fuwuxuanze`, searchForm);
                } else {
                    res = await this.$api.list(`fuwuxuanze`, searchForm);
                }
				// 如果是第一页数据置空
